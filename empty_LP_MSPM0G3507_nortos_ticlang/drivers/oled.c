...
/*
 * MSPM0G3507 移植：原 STM32 标准库 GPIO/RCC 代码已替换为软件 I2C BSP。
 * 软件 I2C 会读取每个字节后的 ACK；任意 NACK 会使 OLED_IsConnected() 返回 false。
 */
...
/* OLED_Init 前为 false；初始化期间所有地址/控制/数据字节均 ACK 才保持 true。 */
...
{
	/* true 表示释放总线，false 表示开漏下拉；位间隔由 BSP 产生。 */
	BSP_OLED_SoftI2C_WriteSCL(BitValue != 0U);
}
...
{
	BSP_OLED_SoftI2C_WriteSDA(BitValue != 0U);
}
...
{
	/* 引脚复用由 SYSCFG_DL_init 完成，本函数只释放软件 I2C 总线。 */
	BSP_OLED_SoftI2C_Init();
}
...
/**
  ...
  * 返 回 值：无
  */
bool OLED_I2C_SendByte(uint8_t Byte)
{
	uint8_t i;
	bool acknowledged;
	...
	/* 第 9 个时钟释放 SDA 并读取从机低电平 ACK。 */
	OLED_W_SDA(1);
	OLED_W_SCL(1);
	acknowledged = !BSP_OLED_SoftI2C_ReadSDA();
	OLED_W_SCL(0);
	return acknowledged;
}
...
void OLED_WriteCommand(uint8_t Command)
{
	...
	g_oledConnected &= OLED_I2C_SendByte((uint8_t)(CAR_OLED_I2C_ADDRESS_7BIT << 1));
	g_oledConnected &= OLED_I2C_SendByte(0x00);	//控制字节：写命令
	g_oledConnected &= OLED_I2C_SendByte(Command);
	OLED_I2C_Stop();				//I2C终止
}
...
void OLED_WriteData(const uint8_t *Data, uint8_t Count)
{
	...
	OLED_I2C_Start();				//I2C起始
	g_oledConnected &= OLED_I2C_SendByte((uint8_t)(CAR_OLED_I2C_ADDRESS_7BIT << 1));
	g_oledConnected &= OLED_I2C_SendByte(0x40);	//控制字节：写显存数据
	/*循环Count次，进行连续的数据写入*/
	for (i = 0; i < Count; i ++)
	{
		g_oledConnected &= OLED_I2C_SendByte(Data[i]);
	}
	OLED_I2C_Stop();				//I2C终止
}
...
/**
  * 函    数：查询最近一次 OLED_Init/写入是否全部收到 ACK
  * 返 回 值：true 表示目前未检测到 NACK，false 表示未配置或设备无应答
  */
bool OLED_IsConnected(void)
{
	return g_oledConnected;
}
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	int16_t Page, Page1;
	int32_t XStart = X;
	int32_t XEnd = (int32_t)X + Width;

	/* 空区域不更新；先裁剪 X，避免原版在 X+Width>128 时越界读取显存。 */
	if (Width == 0U || Height == 0U) {return;}
	if (XStart < 0) {XStart = 0;}
	if (XEnd > 128) {XEnd = 128;}
	...
	{
		if (j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor((uint8_t)j, (uint8_t)XStart);
			/*只发送裁剪后仍位于显存中的列*/
			OLED_WriteData(&OLED_DisplayBuf[j][XStart], (uint8_t)(XEnd - XStart));
		}
	}
}
...
void OLED_ShowSignedNum(int16_t X, int16_t Y, int32_t Number, uint8_t Length, uint8_t FontSize)
{
	...
	{
		OLED_ShowChar(X, Y, '-', FontSize);	//显示-号
		/* 先提升到 int64_t，避免 INT32_MIN 直接取负发生有符号溢出。 */
		Number1 = (uint32_t)(-(int64_t)Number);
	}
...
void OLED_ShowImage(int16_t X, int16_t Y, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
	...
	/* 防止 Height-1 在无符号运算中下溢，也防止空图像指针被访问。 */
	if (Width == 0U || Height == 0U || Image == NULL) {return;}
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, const char *format, ...)
{
	...
	/* 使用有长度上限的版本，避免原版 vsprintf 写出 256 字节栈缓冲。 */
